...
def main():
    a_len = len(sys.argv)
    if a_len < 3:
        print("arguments: [model name] [data dir] [type]") 
        sys.exit(0)
    model_name = sys.argv[1]
    data_dir = sys.argv[2]
    m = None

    if a_len > 3 and sys.argv[3] == 'tune':
        post_m_name = model_name + '.post'
        m = load_model_fine_grain(model_name)
    else:
        post_m_name = model_name
        m = create_model()
        
    history = run_model(m, data_dir) 
    save_model(m, post_m_name) 
    history_file = post_m_name +'.history'  
